...
//2009-7-27
//#include "StdAfx.h"
#define EUVTKOFF_DLLAPI _declspec(dllexport)
...
/*----------------------------------------------------------------
*off file supported by princeton util code 
*/
							  ...
							  vtkInformationVector *outputVector)
{
	...
	{ // (make a local scope section to emphasise that the variables below are only used here)
		...
		char buffer[1024];
         float xyz[3];
		while (everything_ok&&fgets(buffer, 1023, in)!=NULL)
		{
			...
			if (nverts == 0) 
			{
				...
				{ 
					meshnvert++;
				    if (sscanf(bufferp, "%f%f%f", xyz, xyz + 1, xyz + 2)==3) 
				   {
					...
					if (meshnface < nfaces) 
			     {
					 ...
					 vtkIdType* faceIndex = new vtkIdType[nvertsperface];
					 for (int i = 0; i <nvertsperface; i++) 
					 {
						 bufferp = strtok(NULL, " \t");//
						 if (bufferp) 
						 {
							 faceIndex[i] = atoi(bufferp);
						 }
						 else 
						 {
							 vtkErrorMacro(<<"Error in reading file");
					  ...
					  polys->InsertNextCell(nvertsperface,faceIndex);
					 delete []faceIndex;
			      }//if (meshnface < nfaces) 
		}//while
	}
